...
class NodeAllocatableResourceMapping(BaseModel):
    class Builder(BaseModelBuilder):
        @property
        def cls(self) -> type["NodeAllocatableResourceMapping"]:
            return NodeAllocatableResourceMapping

        def build(self) -> "NodeAllocatableResourceMapping":
            return NodeAllocatableResourceMapping(**self._attrs)

        @overload
        def allocation_multiplier(
            self, value_or_callback: Quantity | None, /
        ) -> "NodeAllocatableResourceMapping.Builder": ...
        ...
        def allocation_multiplier(self, value_or_callback=UNSET, /):
            ...
            if self._in_context and value_or_callback is UNSET:
                context = Quantity.BuilderContext()
                context._parent_builder = self
                context._field_name = "allocation_multiplier"
                return context
            ...
            if callable(value_or_callback):
                ...
                if isinstance(output, Quantity.Builder):
                    ...
                    value = output
            return self._set("allocation_multiplier", value)
    ...
    class BuilderContext(BuilderContextBase["NodeAllocatableResourceMapping.Builder"]):
        def model_post_init(self, __context) -> None:
            self._builder = NodeAllocatableResourceMapping.Builder()
            self._builder._in_context = True
            self._parent_builder = None
            self._field_name = None
    ...
    @classmethod
    def new(cls) -> BuilderContext:
        """Creates a new context manager builder for NodeAllocatableResourceMapping."""
        return cls.BuilderContext()

    @classmethod
    def list_builder(
        cls,
    ) -> GenericListBuilder["NodeAllocatableResourceMapping", Builder]:
        return GenericListBuilder[
            NodeAllocatableResourceMapping, NodeAllocatableResourceMapping.Builder
        ]()

    allocation_multiplier: Annotated[Quantity | None, Field(alias="allocationMultiplier")] = None
    'AllocationMultiplier is used as a multiplier for the allocated device count or the allocated capacity in the claim. It defaults to 1 if not specified. How the field is used also depends on whether `capacityKey` is set. 1.  If `capacityKey` is NOT set: `allocationMultiplier` multiplies the device count allocated to the claim.\n\t   a. A DRA driver representing each CPU core as a device would have\n       {ResourceName: "cpu", allocationMultiplier: "2"} in its\n       `nodeAllocatableResourceMappings`. If 4 devices are allocated to the claim,\n\t\t  4 * 2 CPUs would be considered as allocated and subtracted from the node\u0027s capacity.\n    b. A GPU device that needs additional node memory per GPU allocation would\n       have {ResourceName: "memory", allocationMultiplier: "2Gi"}.  Each allocated\n\t\t  GPU device instance of this type will account for 2Gi of memory.\n\n2.  If `capacityKey` IS set: `allocationMultiplier` is multiplied by the amount of that capacity consumed.\n\t   The final node allocatable resource amount is `consumedCapacity[capacityKey]` * `allocationMultiplier`.\n    For example, if a Device\u0027s capacity "dra.example.com/cores" is consumed,\n    and each "core" provides 2 "cpu"s, the mapping would be:\n    {ResourceName: "cpu", capacityKey: "dra.example.com/cores", allocationMultiplier: "2"}.\n    If a claim consumes 8 "dra.example.com/cores", the CPU footprint is 8 * 2 = 16.'
...
class Device(BaseModel):
    class Builder(BaseModelBuilder):
        ...
        def node_allocatable_resource_mappings(
            self, value: dict[str, NodeAllocatableResourceMapping] | None, /
        ) -> Self:
            ...
            return self._set("node_allocatable_resource_mappings", value)
    ...
    name: str
    "Name is unique identifier among all devices managed by the driver in the pool. It must be a DNS label."
    node_allocatable_resource_mappings: Annotated[
        dict[str, NodeAllocatableResourceMapping] | None,
        Field(alias="nodeAllocatableResourceMappings"),
    ] = None
    'NodeAllocatableResourceMappings defines the mapping of node resources that are managed by the DRA driver exposing this device. This includes resources currently reported in v1.Node `status.allocatable` that are not extended resources (see https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#extended-resources). Examples include "cpu", "memory", "ephemeral-storage", and hugepages. In addition to standard requests made through the Pod `spec`, these resources can also be requested through claims and allocated by the DRA driver. For example, a CPU DRA driver might allocate exclusive CPUs or auxiliary node memory dependencies of an accelerator device. The keys of this map are the node-allocatable resource names (e.g., "cpu", "memory"). Extended resource names are not permitted as keys.'
