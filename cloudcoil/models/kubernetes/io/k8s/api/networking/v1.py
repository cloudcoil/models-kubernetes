...
class NetworkPolicyPort(BaseModel):
    class Builder(BaseModelBuilder):
        ...
        @overload
        def port(self, value_or_callback: Never = ...) -> "intstr.IntOrString.BuilderContext": ...
...
class NetworkPolicyPeer(BaseModel):
    ...
    namespace_selector: Annotated[
        Optional[v1_1.LabelSelector], Field(alias="namespaceSelector")
    ] = None
    """
    namespaceSelector selects namespaces using cluster-scoped labels. This field follows standard label selector semantics; if present but empty, it selects all namespaces.
    ...
    If podSelector is also set, then the NetworkPolicyPeer as a whole selects the pods matching podSelector in the namespaces selected by namespaceSelector. Otherwise it selects all pods in the namespaces selected by namespaceSelector.
    """
    pod_selector: Annotated[Optional[v1_1.LabelSelector], Field(alias="podSelector")] = None
    """
    podSelector is a label selector which selects pods. This field follows standard label selector semantics; if present but empty, it selects all pods.
...
class NetworkPolicySpec(BaseModel):
    class Builder(BaseModelBuilder):
        ...
        @overload
        def pod_selector(
            self, value_or_callback: v1_1.LabelSelector, /
        ) -> "NetworkPolicySpec.Builder": ...
    ...
    ingress is a list of ingress rules to be applied to the selected pods. Traffic is allowed to a pod if there are no NetworkPolicies selecting the pod (and cluster policy otherwise allows the traffic), OR if the traffic source is the pod's local node, OR if the traffic matches at least one ingress rule across all of the NetworkPolicy objects whose podSelector matches the pod. If this field is empty then this NetworkPolicy does not allow any traffic (and serves solely to ensure that the pods it selects are isolated by default)
    """
    pod_selector: Annotated[v1_1.LabelSelector, Field(alias="podSelector")]
    """
    podSelector selects the pods to which this NetworkPolicy object applies. The array of ingress rules is applied to any pods selected by this field. Multiple network policies can select the same set of pods. In this case, the ingress rules for each are combined additively. This field is NOT optional and follows standard label selector semantics. An empty podSelector matches all pods in this namespace.
    """
