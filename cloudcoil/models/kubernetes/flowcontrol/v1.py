...
class LimitedPriorityLevelConfiguration(BaseModel):
    class Builder(BaseModelBuilder):
        ...
        @overload
        def limit_response(
            self, value_or_callback: LimitResponse, /
    ...
    lendable_percent: Annotated[int | None, Field(alias="lendablePercent")] = None
    "`lendablePercent` prescribes the fraction of the level\u0027s NominalCL that can be borrowed by other priority levels. The value of this field must be between 0 and 100, inclusive, and it defaults to 0. The number of seats that other levels can borrow from this level, known as this level\u0027s LendableConcurrencyLimit (LendableCL), is defined as follows.\n\nLendableCL(i) = round( NominalCL(i) * lendablePercent(i)/100.0 )"
    limit_response: Annotated[LimitResponse, Field(alias="limitResponse")]
    "`limitResponse` indicates what to do with requests that can not be executed right now"
    nominal_concurrency_shares: Annotated[int | None, Field(alias="nominalConcurrencyShares")] = (
        None
    )
...
class PriorityLevelConfiguration(Resource):
    ...
    class Builder(BaseModelBuilder):
        ...
        @overload
        def spec(
            self, value_or_callback: PriorityLevelConfigurationSpec, /
        ) -> "PriorityLevelConfiguration.Builder": ...
...
class FlowSchema(Resource):
    ...
    class Builder(BaseModelBuilder):
        ...
        @overload
        def spec(self, value_or_callback: FlowSchemaSpec, /) -> "FlowSchema.Builder": ...
